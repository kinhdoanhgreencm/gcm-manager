...
-- 1.3. Thêm cột interior_color nếu bảng đã tồn tại (Migration)
ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS interior_color TEXT;

-- 1.4. Thêm cột engine_number nếu bảng đã tồn tại (Migration)
ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS engine_number TEXT;

-- 1.5. Thêm cột updated_by nếu bảng đã tồn tại (Migration)
...
COMMENT ON COLUMN vehicles.color IS 'Màu ngoại thất của xe';
COMMENT ON COLUMN vehicles.interior_color IS 'Màu nội thất của xe (Đen, Xám, Be, Nâu)';
