...
import { ClaimType, ClaimStatus, ClaimPriority, Vehicle, Customer, Claim } from '@/types';
...
export const ClaimEditPage: React.FC<ClaimEditPageProps> = ({ claimId }) => {
  ...
  // Fetch claim data and related data
  useEffect(() => {
    fetchClaim();
    fetchCustomers();
    fetchVehicles();
  ...
  const fetchClaim = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        setSubmitError('Không tìm thấy hồ sơ claim hoặc có lỗi xảy ra');
        return;
      }

      if (!result?.claim) {
        setSubmitError('Không tìm thấy hồ sơ claim');
        return;
      }

      // Populate form data
      setFormData({
        type: result.claim.type as ClaimType,
        status: result.claim.status as ClaimStatus,
        priority: result.claim.priority as ClaimPriority,
        customerId: result.claim.customerId || '',
        customerName: result.claim.customerName || '',
        customerPhone: result.claim.customerPhone || '',
        customerEmail: result.claim.customerEmail || '',
        vehicleId: result.claim.vehicleId || '',
        vehicleCode: result.claim.vehicleCode || '',
        contractId: result.claim.contractId || '',
        contractCode: result.claim.contractCode || '',
        title: result.claim.title || '',
        description: result.claim.description || '',
        requestedAmount: result.claim.requestedAmount ? result.claim.requestedAmount.toString() : '',
        approvedAmount: result.claim.approvedAmount ? result.claim.approvedAmount.toString() : '',
        resolution: result.claim.resolution || '',
        notes: result.claim.notes || '',
        assignedToId: result.claim.assignedToId || '',
        reportedDate: result.claim.reportedDate || new Date().toISOString().split('T')[0],
        dueDate: result.claim.dueDate || ''
      });
    } catch (err: any) {
      console.error('Error fetching claim:', err);
      setSubmitError('Có lỗi xảy ra khi tải dữ liệu hồ sơ claim');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canUpdateClaim) {
      setSubmitError('Bạn không có quyền sửa hồ sơ claim');
      return;
    }
    ...
    try {
      ...
      // Update resolved_date if status changed to RESOLVED
      if (formData.status === ClaimStatus.RESOLVED) {
        updateData.resolved_date = new Date().toISOString().split('T')[0];
        updateData.resolved_by_id = user?.id || null;
      }

      // Update closed_date if status changed to CLOSED
      if (formData.status === ClaimStatus.CLOSED) {
      ...
      const response = await fetch(`/api/claims/${claimId}`, {
        ...
        body: JSON.stringify({ claim: updateData })
      });
      const result = await response.json();

      if (!response.ok) {
        console.error('Error updating claim:', result?.error || 'Unknown error');
        throw new Error(result?.error || 'Lỗi cập nhật dữ liệu hồ sơ claim');
      }
      ...
      setSubmitError(error.message || 'Có lỗi xảy ra khi cập nhật hồ sơ claim');
    } finally {
  ...
  if (!canUpdateClaim) {
    return <AccessDenied message="Bạn không có quyền sửa hồ sơ claim. Vui lòng liên hệ quản trị viên để được cấp quyền." />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-white mb-2">Sửa hồ sơ claim</h1>
          <p className="text-slate-300/80 text-sm">Cập nhật thông tin khiếu nại, bảo hành hoặc yêu cầu của khách hàng</p>
        </div>
      </div>

      {/* Form - Same structure as ClaimFormPage */}
        ...
        <section className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              >
                <option value={ClaimStatus.PENDING}>Chờ xử lý</option>
                <option value={ClaimStatus.IN_PROGRESS}>Đang xử lý</option>
                <option value={ClaimStatus.RESOLVED}>Đã giải quyết</option>
                <option value={ClaimStatus.REJECTED}>Từ chối</option>
                <option value={ClaimStatus.CLOSED}>Đã đóng</option>
              </select>
            </div>
        ...
        <div className="flex justify-end gap-4 pt-6 border-t border-white/10">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <Save size={18} />
                <span>Cập nhật hồ sơ claim</span>
              </>
            )}
          </button>
        </div>
